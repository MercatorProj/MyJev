...
下面介绍的是 **MyJev 如何用 LLM 实现这种请求**，不代表 Jev 官方服务的内部实现。
...
## MyJev：先将问题构造成独立的 yes/no 判断

MyJev 首先把 Jev Request 中的每个选项转换成一个独立的 LLM 输入，让它判断“这个选项是否符合材料”。完成输入构造后，再读取 LLM 对 yes/no 的分数，计算概率并组装结果。
...
有了上面的候选输入，就可以让 LLM 处理它们。虽然提示词要求回答 yes/no，MyJev 实际并不生成这两个词，而是在处理完输入后，直接读取下一个 token 中 `yes` 和 `no` 的分数（logits），计算：
...
同一问题的候选独立评分，分数之和不一定为 1。MyJev 按题型处理：
