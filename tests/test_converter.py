...
class MyJevTests(unittest.TestCase):
    def test_evaluates_mixed_request_end_to_end(self) -> None:
        ...
        self.assertIn("Candidate: billing\n", prompts[0][1]["content"])
        self.assertIn("Is the answer to the following question no?", prompts[-1][1]["content"])
    ...
    def test_rejects_backend_probability_count_mismatch(self) -> None:
        ...
        with self.assertRaises(ValueError):
            MyJev(backend=FakeBackend([0.2, 0.8])).evaluate(request)
